    ...
    match (username, password) {
        (Ok(username), Ok(password)) => {
            ...
            let firmware: service::DrogueHttp<'_, _, _, 512> =
                service::DrogueHttp::new(TokioTcpSocket, OsRng, addr, host, &username, &password);
...
impl embedded_nal_async::TcpConnect for TokioTcpSocket {
    ...
    fn connect<'m>(&'m self, remote: embedded_nal_async::SocketAddr) -> Self::ConnectFuture<'m> {
        async move {
            let stream = TcpStream::connect(format!("{}:{}", remote.ip(), remote.port())).await?;
